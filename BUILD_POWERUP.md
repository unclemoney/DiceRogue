...
1. Add a print or assert inside the handler to confirm it's called with expected parameters.
2. Trigger the event (e.g., assign a score, complete challenge) and verify the handler output.

**Duplicate PowerUp Triggers:**
1. Grant the PowerUp and a replica (or grant the same PowerUp twice if allowed).
2. Trigger the effect (e.g., complete challenge for `allowance`) and verify both instances activate (e.g., $200 for two Allowance PowerUps).

**Target Verification:**
1. Temporarily add `print("apply target:", target.get_class())` in `apply()` to confirm the correct target type.
2. If target type is incorrect, delegate activation to `GameController` instead of implementing a local mapping.

## Complex PowerUp Example: The Replicator
...
Purchase Replicator
        ↓
 Round completes → Replicator becomes CHARGED (stores chosen_powerup_id)
        ↓
 Player sells Replicator
        ↓
 GameController.grant_replica_power_up(chosen_powerup_id)
        ↓
 Replica created as an independent PowerUp (unique id, UI icon, registered in active_power_ups) — survives after original is removed
```

Key rules

- Charge on round completion; do NOT spawn the replica until the Replicator is sold.
- Spawn replicas via GameController so they are fully independent (registered in `active_power_ups`, added to UI).
- Give the replica a unique id (e.g., `original_id + "_replica_<timestamp>"`) to allow multiple instances and correct lifecycle handling.
- Never parent a replica under the source PowerUp node (it will be freed with the source).
- Ensure GameController activation handles replica suffixes so both original and replica trigger properly.

Example code pattern

```gdscript
# Replicator PowerUp (simplified)
func _on_round_completed() -> void:
    # pick a valid owned powerup id (exclude self + replicas)
    chosen_powerup_id = _pick_random_owned_powerup()
    charged = true
    emit_signal("description_updated", id, get_current_description())

func remove(_target) -> void:
    # called when sold; only spawn replica if it was charged
    if charged and chosen_powerup_id != "":
        game_controller.grant_replica_power_up(chosen_powerup_id)

# GameController method (conceptual)
func grant_replica_power_up(original_id: String) -> void:
    var def = pu_manager.get_def(original_id)
    if not def:
        push_error("grant_replica_power_up: no def for %s" % original_id)
        return
    # duplicate resource and give a unique id
    var replica_def = def.duplicate(true)
    var replica_id = original_id + "_replica_" + str(OS.get_unix_time())
    replica_def.id = replica_id
    # register and spawn (pseudo calls; adapt to project API)
    # Note: In this codebase, use pu_manager.spawn_power_up() + active_power_ups registration + power_up_ui.add_power_up()
    var pu := pu_manager.spawn_power_up(original_id, power_up_container) as PowerUp
    if pu:
        active_power_ups[replica_id] = pu
        var icon_def: PowerUpData = replica_def
        power_up_ui.add_power_up(icon_def)
...
- Test: buy Replicator, complete one round, sell Replicator → replica appears and persists.
- Test: have two of the same PowerUp (original + replica), trigger the effect → confirm both instances activate (e.g., Allowance ×2 pays double).
- Ensure UI tweens use `get_tree().create_tween()` when animations must survive node freeing.
...
**Why:** GameController._activate_power_up() is the authoritative source. Duplicating this logic leads to silent failures when mappings diverge.

**Right:** Delegate activation to the GameController (e.g., `grant_power_up()` or `grant_replica_power_up()`) so the PowerUp receives the correct target and signal wiring.

### ❌ DON'T: Use `Engine.has_singleton(...)` for Autoloads
**Wrong:**
```gdscript
if Engine.has_singleton("ScoreModifierManager"):
    var mgr = Engine.get_singleton("ScoreModifierManager")
```
**Right:**
```gdscript
if ScoreModifierManager != null:
    ScoreModifierManager.register_additive("source", 10)
```
**Why:** Godot autoloads are direct global references; `Engine.has_singleton` returns false for them.

### ❌ DON'T: Parent replica PowerUps under the source PowerUp
**Wrong:**
```gdscript
add_child(replica_instance)  # Replica will be freed when parent is freed
```
**Right:** Spawn and register replicas through GameController (suffix id with `_replica`, create UI data, register in `active_power_ups`) so they are independent and persistent.

### ❌ DON'T: Assume signal parameters without checking
**Wrong:**
```gdscript
score_assigned.connect(_on_score_assigned)

func _on_score_assigned(category: String, score: int):  # Missing 'section' param
    pass
```
**Right:**
```gdscript
func _on_score_assigned(_section, category: String, score: int) -> void:
    # Use underscore prefix for unused params to avoid lint warnings
```

### Tip: UI Tweens and Node Lifecycle
- Use `get_tree().create_tween()` for UI animations that must survive node freeing (tween is attached to the SceneTree, not the node).

### Debugging Tips
- Use `get_tree().get_first_node_in_group("game_controller")` to get the GameController, then access `game_controller.score_card_ui`, `game_controller.turn_tracker`, etc.
- Prefer GameController access over group lookups for reliability and to avoid naming mistakes (e.g., `score_card_ui` vs `scorecard_ui`).
