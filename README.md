...
- **Mall Map Polish**: The board is a lightbox — procedural light grey background with a top-center sheen + vignette (`Scripts/Shaders/mall_lightbox.gdshader`) and a dark grey tiled corridor floor (`Scripts/Shaders/mall_floor_tiles.gdshader`, tunable `tile_size_px`/`grout_width`/`color_a`/`color_b`). Each store has a pictogram plaque (6 per zone, 24 total, `MallStoreIcons` atlas at `Resources/Art/UI/mall_store_icons.png`, built by `MallMapRenderer.build_store_icons` on both screens); icons pop scale + trim glow on hover and squash on press — visual juice only, no gameplay logic. Icon tooltips run through `MallIconTooltipController` (150ms rest delay, 100ms exit grace, neighbor swaps replace content without a hide/reshow). Icon rows are clamped inside their zone polygons (plate + trim + 4px margin). Zones have drop shadows and shader-driven hover/select glow with scale tweens; both screens open with a staged reveal (corridor floor fade → staggered zone pop-in → icon fade). Tooltips use the shared `MallStoreTooltip` component with identical per-store content (name, scaled target, debuffs, status) on both screens. START, the dice-set arrows, and the popup's CLOSE are glass buttons per `Docs/GlassButtonShaderStandard.md` (shared `MallMapRenderer.MALL_GLASS_PALETTE` on `GlassActionButton`); START's locked state uses the shader's `disabled_factor`
- **In-Game Mall Map**: `MallMapPopup` (opened from the VCR tracker's Mall Zone label) renders the same board cropped to the map frame, with the store directory grouped by zone below it via the shared `MallMapRenderer.build_store_directory` (fixed columns, bulleted one-line rows, runtime-measured font step-down). The current store plaque pulses gently
...
- **MallMapLayout** (`Scripts/Managers/mall_map_layout.gd`) - Fixed mall geometry: 4 L-shaped zones around the center diamond courtyard with 4 corridors
- **MallMapRenderer** (`Scripts/Managers/mall_map_renderer.gd`) - Shared board builders (lightbox backdrop, tiled corridor floor, zones, store icons, store directory, shared store tooltip text) and the mall glass-button palette
- **MallStoreIcons** (`Scripts/Managers/mall_store_icons.gd` + `Resources/Art/UI/mall_store_icons.png`) - Store name → atlas cell Dictionary; 24 pictograms + reserved fallback cell
- **MallStoreIcon** (`Scripts/Managers/mall_store_icon.gd`) - One interactive pictogram plaque (hover pop, state tint, current-store pulse)
- **MallStoreTooltip** (`Scripts/UI/mall_store_tooltip.gd`) - Shared tooltip panel component used by both mall screens
- **MallMapZone** (`Scripts/Managers/mall_map_zone.gd`) - Runtime zone node for the selector
- **MallMapPopup** (`Scripts/UI/mall_map_popup.gd`) - In-game mall map modal: board, store plaques with challenge tooltips, store directory
...
- **Rep Stat**: A persistent meta stat (0-100, saved in the profile) that grows with successful sass (+8, defer +7, storm off +9) and shrinks with bootlicking compliance (polite on punishment visits -2, polite on check-ins -1). Rep gates kiosk inventory by POG tier (`REP_TIER_THRESHOLDS = [0, 0, 15, 35, 60]`): G and PG are always open, 15+ unlocks PG-13 ("Parental Guidance"), 35+ R ("Grounded"), 60+ NC-17 ("Banned"). Pacing is now tuned so Rep 60 (tier 4, NC-17 POGs) is reachable by the end of Mall Zone 2 Round 6 for a sass-heavy run; the common neutral check-in was retuned so the target is based on real expected gains, not only best-case storm-off chains. Below Rep 15 with a happy Mom (mood ≤ 3), the Mom-Approved (G) pool gets 1.5x shelf weight and a 10% discount instead — two playstyles, two item pools. Rep-gated items appear greyed in the shop's LOCKED tab with their Rep requirement.
...
7. **Bird Feeder Fever** — Mom goes all-in on premium seed and a cardinal spreadsheet → kitchen-window stakeout mode → the cardinal finally lands and she pays out a $25 victory allowance.
8. **The Squirrel Campaign** — Chairman Nibbles raids the yard → he outsmarts the fancy baffle → Mom wins with decoy peanuts and celebrates with a $30 shop stipend.
9. **Derek's Room Disaster** — a shared mess turns Derek's room into a biohazard → the blame game reaches the air vent → real cleanup earns $30 and a temporary peace treaty.
10. **The Missing Casserole Dish** — Debra escalates a missing-dish panic into neighborhood paperwork → Patterson counter-labels the ladle → the peace offering comes back as a $25 payout to you.
...
- **Toggle Mode**: Set `toggle_mode = true` for checkbox-style buttons; emits `toggled(is_toggled)` and supports `set_toggled(value, should_emit)`. Selected state holds a glow baseline on the shader. CarryOverPanel rows indicate selection by swapping to a brighter per-type palette (`set_palette`), scaling the button up slightly, restoring full alpha (unselected rows stay dimmed at `UNSELECTED_ALPHA`), and prefixing the label with "✔ " — the row layout stays centered and never shifts
