extends PowerUp
class_name ModMoneyPowerUp

## ModMoneyPowerUp
##
## Common PowerUp that grants $8 for each die with a mod when scoring.
## Synergizes with mods to provide passive economy boost.
## Price: $75, Rarity: Common

const MONEY_PER_MODDED_DIE: int = 8

var scorecard_ref: Scorecard = null
var score_card_ui_ref = null
var total_mod_money_earned: int = 0
...
func apply(target) -> void:
	print("=== Applying ModMoneyPowerUp ===")
	var scorecard = target as Scorecard
	if not scorecard:
		push_error("[ModMoneyPowerUp] Target is not a Scorecard")
		return
	...
	if game_controller and game_controller.score_card_ui:
		...
		push_error("[ModMoneyPowerUp] Could not find ScoreCardUI via GameController")
	
	if not is_connected("tree_exiting", _on_tree_exiting):
		connect("tree_exiting", _on_tree_exiting)

func remove(_target) -> void:
	print("=== Removing ModMoneyPowerUp ===")
...
func _on_about_to_score(_section: Scorecard.Section, _category: String, _dice_values: Array[int]) -> void:
	# Count dice with mods in the current hand
...
func get_current_description() -> String:
	return "Earn $%d per modded die when scoring.\nTotal earned: $%d" % [MONEY_PER_MODDED_DIE, total_mod_money_earned]
...
func _update_power_up_icons() -> void:
	...
	if power_up_ui:
		var icon = power_up_ui.get_power_up_icon("mod_money")
		if icon:
