extends Node
#class_name MultiplierManager

# Singleton pattern for managing all score multipliers in the game
# This centralizes multiplier handling from PowerUps, Debuffs, etc.
...
func _ready() -> void:
	...
	if _debug_enabled:
		print("[MultiplierManager] Singleton initialized")
...
func register_multiplier(source_name: String, multiplier: float) -> void:
	if _debug_enabled:
		print("[MultiplierManager] Registering multiplier from '", source_name, "': ", multiplier)
	
	# Validate multiplier value
	if multiplier <= 0:
		push_error("[MultiplierManager] Invalid multiplier value: " + str(multiplier) + " from " + source_name)
		return
	
	# Validation warning for extremely high multipliers
	if multiplier > 100:
		push_warning("[MultiplierManager] Very high multiplier detected: " + str(multiplier) + " from " + source_name)
	...
	if _debug_enabled:
		if was_updating:
			print("[MultiplierManager] Updated existing multiplier '", source_name, "' from ", old_value, " to ", multiplier)
		else:
			print("[MultiplierManager] Added new multiplier '", source_name, "'")
		print("[MultiplierManager] Total multiplier changed from ", old_total, " to ", new_total)
		print("[MultiplierManager] Active multipliers: ", _active_multipliers)
...
func unregister_multiplier(source_name: String) -> void:
	if not _active_multipliers.has(source_name):
		if _debug_enabled:
			print("[MultiplierManager] Warning: Attempted to unregister non-existent multiplier: ", source_name)
		return
	...
	if _debug_enabled:
		print("[MultiplierManager] Unregistered multiplier from '", source_name, "': ", removed_multiplier)
		print("[MultiplierManager] Total multiplier changed from ", old_total, " to ", new_total)
		print("[MultiplierManager] Active multipliers: ", _active_multipliers)
...
# Reset all multipliers (for new games/rounds)
func reset() -> void:
	if _debug_enabled:
		print("[MultiplierManager] RESET CALLED - STACK TRACE:")
		print(get_stack())
		print("[MultiplierManager] Resetting all multipliers")
		print("[MultiplierManager] Clearing ", _active_multipliers.size(), " active multipliers")
	
	var old_total = get_total_multiplier()
	_active_multipliers.clear()
	
	if _debug_enabled:
		print("[MultiplierManager] All multipliers cleared, total multiplier reset to 1.0")
...
func set_debug_enabled(enabled: bool) -> void:
	...
	if enabled:
		print("[MultiplierManager] Debug logging enabled")
	else:
		print("[MultiplierManager] Debug logging disabled")
...
func _validate_state() -> bool:
	...
	for multiplier in _active_multipliers.values():
		if multiplier <= 0:
			push_error("[MultiplierManager] VALIDATION ERROR: Invalid multiplier found: " + str(multiplier))
			return false
	...
	if abs(calculated_total - stored_total) > 0.001:  # Allow for floating point precision
		push_error("[MultiplierManager] VALIDATION ERROR: Total multiplier mismatch. Calculated: " + str(calculated_total) + ", Stored: " + str(stored_total))
		return false
