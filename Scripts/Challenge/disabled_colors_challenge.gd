extends Challenge
class_name DisabledColorsChallenge

var _scorecard: Scorecard = null
var _game_controller: GameController = null
var _debuff_id: String = "disabled_colors"

func _ready() -> void:
	add_to_group("challenges")
	print("[DisabledColorsChallenge] Ready")

## apply(_target)
##
## Sets up the challenge requirements: 220 point target and disabled_colors debuff.
## Connects to scorecard signals to track progress.
func apply(_target) -> void:
	_game_controller = _target as GameController
	if _game_controller:
		print("[DisabledColorsChallenge] Applied to game controller")
		
		# Get scorecard reference
		_scorecard = _game_controller.scorecard
		if not _scorecard:
			push_error("[DisabledColorsChallenge] Failed to get scorecard reference")
			return
			
		# Connect to score change signals
		if _scorecard.has_signal("score_changed"):
			print("[DisabledColorsChallenge] Found score_changed signal")
			if not _scorecard.is_connected("score_changed", _on_score_changed):
				_scorecard.score_changed.connect(_on_score_changed)
				print("[DisabledColorsChallenge] Connected to score_changed signal")
			else:
				print("[DisabledColorsChallenge] Already connected to score_changed signal")
		else:
			push_error("[DisabledColorsChallenge] Scorecard does not have signal score_changed")
		
		# Connect to game completion signal
		if _scorecard.has_signal("game_completed"):
			print("[DisabledColorsChallenge] Found game_completed signal")
			if not _scorecard.is_connected("game_completed", _on_game_completed):
				_scorecard.game_completed.connect(_on_game_completed)
				print("[DisabledColorsChallenge] Connected to game_completed signal")
			else:
				print("[DisabledColorsChallenge] Already connected to game_completed signal")
		else:
			push_error("[DisabledColorsChallenge] Scorecard does not have signal game_completed")
		
		# Apply the disabled_colors debuff
		_game_controller.apply_debuff(_debuff_id)
		print("[DisabledColorsChallenge] Disabled colors debuff enabled")
		
		# Update initial progress
		_update_progress()
	else:
		push_error("[DisabledColorsChallenge] Invalid target - expected GameController")
...
func remove() -> void:
	if _game_controller:
		# Remove the disabled_colors debuff
		if _game_controller.is_debuff_active(_debuff_id):
			_game_controller.disable_debuff(_debuff_id)
			print("[DisabledColorsChallenge] Disabled colors debuff disabled")
...
func _update_progress() -> void:
	...
	print("[DisabledColorsChallenge] Progress updated:", progress)
...
func _on_score_changed(total_score: int) -> void:
	print("[DisabledColorsChallenge] Score changed event received! New total:", total_score)
	_update_progress()
	
	# Check if we've met the target
	if total_score >= _target_score:
		print("[DisabledColorsChallenge] Target score reached! Challenge completed.")
...
func _on_game_completed() -> void:
	if _scorecard and _scorecard.get_total_score() >= _target_score:
		print("[DisabledColorsChallenge] Game completed with target score reached!")
		emit_signal("challenge_completed")
	else:
		print("[DisabledColorsChallenge] Game completed but target score not reached.")
...
func set_target_score_from_resource(resource: ChallengeData, _round_number: int) -> void:
	if resource:
		...
		print("[DisabledColorsChallenge] Target score set from resource:", _target_score)
